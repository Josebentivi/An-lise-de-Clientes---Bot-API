...
Streamlit analytics dashboard for the JurisAI platform with strict module separation:
...
- **`scriptsql.sql`**, **`resultados.txt`**: Reference/demo artifacts, not part of the Streamlit runtime.
...
- **Feature classification**: Keyword-based via `FEATURE_RULES` list in `analytics.py`; action strings in Portuguese
...
1. **Visao Geral**: KPI cards, growth charts, credit distribution
2. **Retencao**: Retention histogram, cohort heatmap, segmentation
